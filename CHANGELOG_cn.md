...
- 通过 `langparse.__version__` 和 `langparse --version` 统一暴露已安装版本，
  版本号以包元数据为唯一事实源。
- 新增原创 LangParse 标识，提供可缩放 SVG 和 PNG 版本。

### 变更

- 将产品定位明确为两条互补主线：易用的通用文档解析工具集，以及精确、可追溯的
  Excel 结构理解。
- 中英文 README 改为优先介绍安装、快速使用和丰富工作簿结果，不再以引擎对比为主线。
